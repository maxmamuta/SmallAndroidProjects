    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case android.R.id.text1:
                Log.e("Click", "Click on item "+view.getTag());
                if (token != null) {
                    stackoverflowAPI.postUpvoteOnAnswer((Integer) view.getTag(), token, getString(R.string.key), "stackoverflow.com", false, "default").enqueue(upvoteCallback);
                } else {
                    Toast.makeText(this, "You need to authenticate first", Toast.LENGTH_LONG).show();
                }
                break;
            case R.id.authenticate_button:
                startActivityForResult(new Intent(this, Webview.class), 1);
                break;
        }
    }
    ...
    Callback<ListWrapper<Question>> questionsCallback = new Callback<ListWrapper<Question>>() {
        ...
        public void onResponse(Call<ListWrapper<Question>> call, Response<ListWrapper<Question>> response) {
            if(response.isSuccessful()) {
                ListWrapper<Question> question = response.body();
                ArrayAdapter<Question> arrayAdapter = new ArrayAdapter<Question>
                        (MainActivity.this, android.R.layout.simple_spinner_dropdown_item, question.items);
